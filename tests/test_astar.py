...
def test_partial_steps():
    path = AStarStrategy.calculate_path((0,0), (4,4), n_steps=3)
    assert len(path) == 3
    assert path == [(1,0), (1,0), (1,0)]

def test_same_position():
    path = AStarStrategy.calculate_path((2,2), (2,2))
    assert path == []

def test_invalid_start():
    path = AStarStrategy.calculate_path((-1,0), (4,4))
    assert path == []

def test_invalid_target():
    path = AStarStrategy.calculate_path((0,0), (9,9))
    assert path == []